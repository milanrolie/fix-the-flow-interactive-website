...
const input = document.querySelector(".desktop-search-container input");
...
function returnSearch() {
  ...
  fetch("https://knetters.github.io/coding-the-curbs/scripts/smartzones.json")
    ...
    .then(data => {
      ...
      if (searchInput != "") {
        let matchingSmartzones = smartzones.filter(smartzone => 
          smartzone.name.toLowerCase() === searchInput.toLowerCase());

        if (matchingSmartzones) {
          if (result_listEl.className.indexOf('active') !== -1) {
          ...
          result_listEl.innerHTML = "<p class='small-text'>ZOEKRESULTAAT</p>";
          result_listEl.innerHTML = "<p>Geen resultaat</p>";
        }
      }
      else {
        if (desktop_listEl.className.indexOf('active') !== -1) {
        desktop_listEl.classList.toggle("active")
        result_listEl.classList.toggle("active")
        }
      }
